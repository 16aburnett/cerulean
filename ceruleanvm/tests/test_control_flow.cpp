...
TEST_CASE (test_control_flow_conditional) {
    std::vector<uint8_t> bytecode = {
        Opcode::LUI,     0x00, 0x0a, 0x00, // [0x00] r0.0 <- 10
        Opcode::LLI,     0x00, 0x00, 0x00, // [0x04] r0.1 <- 0
        Opcode::LUI,     0x10, 0x0f, 0x00, // [0x08] r1.0 <- 15
        Opcode::LLI,     0x10, 0x00, 0x00, // [0x0c] r1.1 <- 0
        Opcode::LUI,     0x20, 0x20, 0x00, // [0x10] r2.0 <- cond_end
        Opcode::LLI,     0x20, 0x00, 0x00, // [0x14] r2.1 <- 0
        Opcode::BLE,     0x01, 0x20, 0x00, // [0x18] BLE r0, r1, r2(cond_end) // if r0 <= r1, jump to cond_end
        // cond_true: if r0 > 15
        Opcode::LUI,     0x00, 0x01, 0x00, // [0x1c] r0.0 <- 1
        // cond_end:
    ...
    REQUIRE (vm.get_register (0) == 10);
}

TEST_CASE (test_control_flow_loop) {
    std::vector<uint8_t> bytecode = {
        Opcode::LUI,     0x00, 0x00, 0x00, // [0x00] r0.0 <- 0 ; i = 0
        Opcode::LLI,     0x00, 0x00, 0x00, // [0x04] r0.1 <- 0
        Opcode::LUI,     0x10, 0x0a, 0x00, // [0x08] r1.0 <- 10 ; N = 10
        Opcode::LLI,     0x10, 0x00, 0x00, // [0x0c] r1.1 <- 0
        Opcode::LUI,     0x20, 0x40, 0x00, // [0x10] r2.0 <- loop_end
        Opcode::LLI,     0x20, 0x00, 0x00, // [0x14] r2.1 <- 0
        Opcode::LUI,     0x30, 0x30, 0x00, // [0x18] r3.0 <- loop_cond
        Opcode::LLI,     0x30, 0x00, 0x00, // [0x1c] r3.1 <- 0
        Opcode::LUI,     0x40,  '*', 0x00, // [0x20] r4.0 <- '*'
        Opcode::LLI,     0x40, 0x00, 0x00, // [0x24] r4.1 <- 0
        Opcode::LUI,     0x50, '\n', 0x00, // [0x28] r5.0 <- '\n'
        Opcode::LLI,     0x50, 0x00, 0x00, // [0x2c] r5.1 <- 0
        // loop_cond:
    ...
    REQUIRE (vm.get_register (0) == 10); // Ensure i == N
}

# CeruleanRISC Backend - Multi-Pass Architecture

## Overview

The CeruleanRISC backend uses a clean multi-pass compilation pipeline to convert CeruleanIR (SSA form) to CeruleanRISC (physical machine code).
...
[Pass 1: Lowering]
    ↓
Virtual CeruleanRISC (SSA, unlimited vregs)
    ↓
...
[Pass 3: Register Allocation]
    ↓
Virtual CeruleanRISC (physical regs + spill info)
    ↓
[Pass 4: Frame Lowering]
    ↓
Final CeruleanRISC (with stack frames)
    ↓
...
**Output**: Virtual CeruleanRISC AST
...
The `CodeGenVisitor_CeruleanRISC` class serves as the orchestrator that:
...
python3 -m backend.ceruleanIRCompiler input.ceruleanir --target ceruleanrisc

# Use linear scan allocator (optimized, requires proper CFG)
python3 -m backend.ceruleanIRCompiler input.ceruleanir --target ceruleanrisc --regalloc linear-scan
```
...
├── ceruleanVirtualRISCAST.py       # Virtual ASM AST definitions
├── ceruleanVirtualRISCASTVisitor.py # Visitor base class

...
# By Amy Burnett
# =================================================================================================
# Final pass: Converts the Virtual CeruleanRISC AST (with register allocation and frame info)
...
LIB_FILENAME = os.path.dirname(__file__) + "/CeruleanIR_BuiltinLib.crisc"

# =================================================================================================

class ASMEmitter:
    """
    Emits textual assembly code from the Virtual CeruleanRISC AST.
    ...
    def emit(self, finalASM):
        ...
        Args:
            finalASM: Virtual CeruleanRISC AST with register allocation and frame info
...
class EmitterVisitor(ASMASTVisitor):
    ...
    def visitProgramNode(self, node):
        ...
        self.emitter.debugPrint(f"Emitting program with {len(node.globalVars)} global variables")
        
        self.emitComment("CeruleanRISC code compiled from CeruleanIR")
        self.emit("// " + "=" * 75 + "\n\n")
        ...
        # Emit global variables to data section
        if node.globalVars:
            self.emitter.debugPrint(f"Emitting {len(node.globalVars)} global variables to data section")
            for globalVar in node.globalVars:
                self.visit(globalVar)

...
CeruleanRISCVM::CeruleanRISCVM (const std::vector<uint8_t>& bytecode, bool debug_)
...
CeruleanRISCVM::~CeruleanRISCVM () {}

uint64_t CeruleanRISCVM::getRegister (uint8_t index) const {
    return registers.get<uint64_t>(index);
}

uint64_t CeruleanRISCVM::getPC () const {
    return pc;
}

void CeruleanRISCVM::run () {
    if (debug) 
...
void CeruleanRISCVM::step () {
    execute_instruction ();
}

bool CeruleanRISCVM::isHalted () {
    return pc >= code.size ();
}
